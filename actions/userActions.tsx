...
export const GET_USERS_WITH_UNVERIFIED_DOCUMENTS_SUCCESS = 'GET_USERS_WITH_UNVERIFIED_DOCUMENTS_SUCCESS';
export const GET_USERS_WITH_UNVERIFIED_DOCUMENTS_FAILURE = 'GET_USERS_WITH_UNVERIFIED_DOCUMENTS_FAILURE';

const allUsersWithUnverifiedDocuments = () => ({
    [CALL_API]: {
        httpAction: 'GET',
        types: [GET_USERS_WITH_UNVERIFIED_DOCUMENTS, GET_USERS_WITH_UNVERIFIED_DOCUMENTS_SUCCESS,
...
const orderHistoryForUser = (userId) => ({
    [CALL_API]: {
        httpAction: 'GET',
        types: [GET_ORDER_HISTORY_FOR_USER, GET_ORDER_HISTORY_FOR_USER_SUCCESS, GET_ORDER_HISTORY_FOR_USER_FAILURE],
...
const allUserIdDocuments = (userId) => ({
    [CALL_API]: {
        httpAction: 'GET',
        types: [GET_ALL_USER_ID_DOCUMENTS, GET_ALL_USER_ID_DOCUMENTS_SUCCESS, GET_ALL_USER_ID_DOCUMENTS_FAILURE],
...
export const GET_ALL_USER_MEDICAL_RECOMMENDATIONS_SUCCESS = 'GET_ALL_USER_MEDICAL_RECOMMENDATIONS_SUCCESS';
export const GET_ALL_USER_MEDICAL_RECOMMENDATIONS_FAILURE = 'GET_ALL_USER_MEDICAL_RECOMMENDATIONS_FAILURE';

const allUserMedicalRecommendations = (userId) => ({
    [CALL_API]: {
        httpAction: 'GET',
        types: [GET_ALL_USER_MEDICAL_RECOMMENDATIONS, GET_ALL_USER_MEDICAL_RECOMMENDATIONS_SUCCESS, GET_ALL_USER_MEDICAL_RECOMMENDATIONS_FAILURE],
...
export const SET_CURRENT_USER_MEDICAL_RECOMMENDATION_SUCCESS = 'SET_CURRENT_USER_MEDICAL_RECOMMENDATION_SUCCESS';
export const SET_CURRENT_USER_MEDICAL_RECOMMENDATION_FAILURE = 'SET_CURRENT_USER_MEDICAL_RECOMMENDATION_FAILURE';


const setMedicalRecommendation = (documentId: number) => ({
    [CALL_API]: {
        httpAction: 'POST',
        types: [SET_CURRENT_USER_MEDICAL_RECOMMENDATION, SET_CURRENT_USER_MEDICAL_RECOMMENDATION_SUCCESS, SET_CURRENT_USER_MEDICAL_RECOMMENDATION_FAILURE],
...
const dsprTransfer = () => {
    const transfer = { oldDSPR: { id: 2 }, newDeliveryServiceProvider: { id: 3 }, newDSPRName: 'San Diego' };
    return {
        [CALL_API]: {
            httpAction: 'POST',
...
const userDocumentVerification = (userId, documentId, documentType, document) => {
    ...
    return {
        [CALL_API]: {
            ...
            schema: documentType === USER_ID_DOCUMENT ? Schemas.USER_ID_DOCUMENT : Schemas.USER_MEDICAL_RECOMMENDATION,
            body: document
        }
    }
};
...
const adminUserInfoChanger = (userId, newFirstName, newLastName, newPhoneNumber) => {
    ...
    return {
        [CALL_API] : {
            httpAction: 'POST',
            types: [ADMIN_CHANGES_USER_DETAILS,ADMIN_CHANGES_USER_DETAILS_SUCCESS,ADMIN_CHANGES_USER_DETAILS_FAILURE],
...
export const adminChangesUserInfo = (userId, newFirstName, newLastName, newPhoneNumber) => (dispatch, getState) => {
...
const adminPasswordChanger = (userId, newPassword) => {
    ...
    return {
        [CALL_API]: {
            httpAction: 'POST',
            types: [ADMIN_CHANGES_PASSWORD, ADMIN_CHANGES_PASSWORD_SUCCESS, ADMIN_CHANGES_PASSWORD_FAILURE],
...
export const uploadUserDocument = (userId, docType, imageFile) => (dispatch, getState) => {
    return dispatch(userDocumentUploader(userId, docType, imageFile))
        .then(() => dispatch(getSpecificUser(userId)));
};
...
const userNoteCreator = (userId, note, dsprDriverId, dsprManagerId) => {
    ...
    if (dsprDriverId) {
        if (dsprManagerId) throw new Error('Cannot have both dsprDriverId and dsprManagerId when creating a userNote');
        userNote['dsprDriver'] = { id: dsprDriverId };
    }
    else {
        if (dsprManagerId == null) throw new Error('Must have one of dsprDriverId or dsprManagerId set when creating a userNote');
...
export const createUserNote = (userId, note, dsprDriverId, dsprManagerId) => (dispatch, getState) => {
