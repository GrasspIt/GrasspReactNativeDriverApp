...
const callApi = (httpAction: string, endpoint: string, schema, accessToken,
    body: any = {}, queryParamsMap: any = {}, file) => {
...
const searchUserSchema = new schema.Entity('searchUsers', {}, {
    idAttribute: user => user.id,
});
const unverifiedUserSchema = new schema.Entity('unverifiedUsers', {}, {
    idAttribute: user => user.id
});

const dspSchema = new schema.Entity('deliveryServiceProviders', {}, {
    idAttribute: deliveryServiceProvider => deliveryServiceProvider.id
});

const dspManagerSchema = new schema.Entity('dspManagers', {}, {
    idAttribute: dspManager => dspManager.id
});

const dsprSchema = new schema.Entity('DSPRs', {}, {
    idAttribute: dspr => dspr.id
});

const dsprManagerSchema = new schema.Entity('dsprManagers', {}, {
    idAttribute: dsprManager => dsprManager.id
});

const dsprDriverSchema = new schema.Entity('dsprDrivers', {}, {
    idAttribute: dsprDriver => dsprDriver.id
});

const dspProductSchema = new schema.Entity('dspProducts', {}, {
    idAttribute: dspProduct => dspProduct.id
});

const searchProductSchema = new schema.Entity('searchProducts', {}, {
    idAttribute: searchProduct => searchProduct.id
});

const dspProductCategorySchema = new schema.Entity('dspProductCategories', {}, {
    idAttribute: category => category.id
});

const dsprDriverLocationSchema = new schema.Entity('dsprDriverLocations', {}, {
    idAttribute: location => location.id
});

const dsprDriverInventoryPeriodSchema = new schema.Entity('dsprDriverInventoryPeriods', {}, {
    idAttribute: inventoryPeriod => inventoryPeriod.id
});

const dsprZipCodeSchema = new schema.Entity('dsprZipCodes', {}, {
    idAttribute: dsprZipCode => dsprZipCode.id
});

const dsprDriverInventoryItemSchema = new schema.Entity('dsprDriverInventoryItems', {}, {
    idAttribute: item => item.id
});

const dsprProductInventoryTransactionSchema = new schema.Entity('dsprProductInventoryTransactions', {}, {
    idAttribute: transaction => transaction.id
});

const dsprCurrentInventoryItemSchema = new schema.Entity('dsprCurrentInventoryItems', {}, {
    idAttribute: item => item.id
});

const userIdDocumentSchema = new schema.Entity('usersIdDocuments', {}, {
    idAttribute: document => document.id
});

const userMedicalRecommendationSchema = new schema.Entity('usersMedicalRecommendations', {}, {
    idAttribute: document => document.id
});


const dsprProductPriceHistorySchema = new schema.Entity('dsprProductPriceHistories', {}, {
...
const dsprOrderHistorySchema = new schema.Entity('dsprOrderHistories', {}, {
    idAttribute: orderHistory => orderHistory.dspr.id
});

const textBlastSchema = new schema.Entity('textBlasts', {}, {
    idAttribute: textBlast => textBlast.id
});

const userNoteSchema = new schema.Entity('userNotes', {}, {
    idAttribute: userNote => userNote.id
});

const dsprProductCategoryPromotionSchema = new schema.Entity('dsprProductCategoryPromotions', {}, {
    idAttribute: productCategoryPromotion => productCategoryPromotion.id
});

const metricSchema = new schema.Entity('metrics', {}, {
    idAttribute: metric => metric.metric
})

const dsprDriverServiceAreaSchema = new schema.Entity('dsprDriverServiceAreas', {}, {
    idAttribute: dsprDriverServiceArea => dsprDriverServiceArea.id
})

const dsprDriverServiceAreaVertexSchema = new schema.Entity('dsprDriverServiceAreaVertices', {}, {
